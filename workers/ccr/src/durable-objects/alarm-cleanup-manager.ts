...
export async function handleAlarm(deps: AlarmCleanupDeps): Promise<void> {
  ...
  if (metricsEnabled) {
    incrementCounter('ccr_do_alarm_fired_total');
  }
  ...
  if (nextAlarmTime !== null) {
    ...
    if (metricsEnabled) {
      incrementCounter('ccr_do_alarm_decision_total', { decision: 'rescheduled' });
    }
  } else {
    console.log('Cleanup alarm not rescheduled: DO is idle with no data to track');
    if (metricsEnabled) {
      incrementCounter('ccr_do_alarm_decision_total', { decision: 'stopped' });
    }
  }
}
...
export async function scheduleCleanupAlarm(deps: AlarmCleanupDeps): Promise<void> {
  ...
  if (deps.getCleanupAlarmScheduled()) {
    if (metricsEnabled) {
      incrementCounter('ccr_do_alarm_schedule_total', { result: 'skipped_cached' });
    }
    return;
  }
  ...
  if (existingAlarm !== null) {
    deps.setCleanupAlarmScheduled(true);
    if (metricsEnabled) {
      incrementCounter('ccr_do_alarm_schedule_total', { result: 'skipped_existing' });
    }
    return;
  }
  ...
  deps.setCleanupAlarmScheduled(true);
  if (metricsEnabled) {
    incrementCounter('ccr_do_alarm_schedule_total', { result: 'scheduled' });
  }
}
...
function recordAlarmMetrics(
  ...
  recordTiming('ccr_do_cleanup_duration_ms', Date.now() - startTime);

  if (cleanupStats.presenceCleared > 0) {
    incrementCounter('ccr_do_cleanup_items_total', { type: 'presence' }, cleanupStats.presenceCleared);
  }
  if (cleanupStats.focusCleared > 0) {
    incrementCounter('ccr_do_cleanup_items_total', { type: 'focus' }, cleanupStats.focusCleared);
  }
  if (cleanupStats.sessionsCleared > 0) {
    incrementCounter('ccr_do_cleanup_items_total', { type: 'edit_session' }, cleanupStats.sessionsCleared);
  }
  if (cleanupStats.regionsCleared) {
    incrementCounter('ccr_do_cleanup_items_total', { type: 'active_regions' });
  }

  setGauge('ccr_do_connections_count', deps.getConnectionCount());
  setGauge('ccr_do_presence_count', deps.presenceManager.count());
  setGauge('ccr_do_edit_sessions_count', deps.editSessions.size);
  setGauge('ccr_do_active_regions_count', deps.activityDetector.getActiveRegions().length);
  setGauge('ccr_do_focus_regions_count', deps.activityDetector.getHumanFocusRegions().length);
}
