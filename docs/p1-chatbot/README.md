...
When a comment mentions an agent, CCR posts `{ siteId, threadId, commentId, agentIds, requestedBy }` to the worker's `/notifications/comment` with the shared secret in `X-Internal-Secret`. `requestedBy` is the poster (`{ id, email, name }`) when the site roster knows their email. The worker acknowledges with 202, then, if its own `AGENT_ID` is among `agentIds`, reads the thread back through the CCR API acting for that poster (the `X-Acting-User-*` headers) and posts an `agent_activity` comment with `status: 'working'` so the thread shows it is on the job. It then reads the pinned page when the thread names a branch and document, runs one model completion over the thread (with the page outline and pinned block when available), and updates that same comment in place: a `propose_document_edits` tool call becomes an `agent_proposal` (summary plus dot-path operations the editor applies on accept), plain text becomes a `message`, and an error or empty result flips it to `agent_activity` with `status: 'failed'`. Every state is attributed to the agent on the poster's behalf; the proposal is never applied by the worker. Acting for the poster is what lets a global agent in: its implicit site access is bounded by the user it acts for, so a notification without `requestedBy` leaves the agent with no access and the comment is logged as failed. A re-delivered notification finds the agent's comment already in the thread and posts nothing.

Both sides need the same secret: `AGENT_NOTIFY_SECRET` in the worker and in CCR. Locally that means `workers/p1-agent/.env.local` and `workers/ccr/.dev.vars`. In a deployed lane both ends are set by the deploy workflow from one Secret Manager id, `P1_AGENT_NOTIFY_SECRET` — the `ccr` and `p1-agent` jobs read the same id, so the two ends cannot drift apart. Adding a new lane means adding that id to the lane's `terraform/bootstrap/<lane>/main.tf` and putting a value in it; Terraform creates the container, not the value.

CCR finds the worker through `AGENT_WORKER_URL` in its `wrangler.jsonc` vars; leaving either unset disables the dispatch, and a failed delivery is logged and dropped without affecting the comment.
