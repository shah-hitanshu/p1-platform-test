...
- Paths updated: `workers` → `workers/ccr` (imported as `workers/collaborative-state`),
  `workers/mcp-server` → `workers/ccr-mcp-server`, `frontend` → `apps/css-frontend`, media `worker` →
