/**
 * CCR MCP Server - Cloudflare Worker Entry Point
 *
...
interface UserProps {
  ...
  email: string;
  name?: string;
  /** Auth0 access token — forwarded to the CCR backend as Bearer on every API call */
...
const mcpApiHandler: ExportedHandler<Env> & { fetch: NonNullable<ExportedHandler<Env>['fetch']> } = {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    // PCC-3193: emit a one-shot cold-start log of which CCR_BACKEND mode is in
