...
|---|---|---|
| **Run the full P1 stack locally** (Postgres + CSS worker + starter app) | [x] | `pnpm dev:stack` (one command; starter app uses the root `.env.fullstack.local` profile) |
...
| Concept | Command |
|---|---|
| Full P1 stack locally (CSS with mock auth + starter app on the local backend) | `pnpm dev:stack` — brings up Postgres, then runs the CSS worker (:8787) and starter app (:3000) together via turbo. One-time setup: copy `.env.fullstack.example` → `.env.fullstack.local` (both at repo root). On a fresh database, create a site and an API token via the worker API (see `docs/css/`), then put that site id + token in the profile. Ctrl-C stops the servers; `make docker-down` stops Postgres |
...
CSS backend local dev (Postgres via Docker): `make dev`, then `make worker-dev`. See the per-package docs under `docs/` for everything else.
