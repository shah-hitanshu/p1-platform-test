...
import { Icon, Toaster } from '@pantheon-systems/pds-toolkit-react';
...
function P1SubheaderBridgeInner({
  ...
  const handlePublish = options.onPublish ?? (async () => { await publishDocument(); });
...
function P1SubheaderBridge({ options }: { options: CSSPluginOptions }): React.ReactElement | null {
  // Read CSS context — may throw if not inside CSSPuckProvider
  let cssContext: ReturnType<typeof useCSSPuck> | null = null;
  try {
    cssContext = useCSSPuck();
  } catch {
    return null;
  }
  ...
  // Delegate to inner component where all hooks are called unconditionally
...
/**
 * Attaches hover listeners to the Blocks, Outline, AI, and Articles nav tabs
 * and renders a fixed-position portal tooltip for each, bypassing Puck's
 * overflow:hidden nav container.
 *
 * Nav tab order (Puck internals + our plugin array):
 *   0 blocksPlugin  → 'Components'
 *   1 outlinePlugin → 'Outline'
 *   2 CSS plugin    → (NavIcon handles this)
 *   3 AI plugin     → 'AI Builder'
 *   4 PCC plugin    → 'Articles'
 */
const NAV_TOOLTIPS: Record<number, string> = {
  0: 'Components',
  1: 'Outline',
  3: 'AI Builder',
  4: 'Articles',
...
function NavTooltipBridge(): React.ReactElement {
  ...
  useEffect(() => {
    ...
    function setup() {
      const navList = document.querySelector('[class*="Nav-list"]');
      if (!navList) return;

      const items = navList.querySelectorAll(':scope > li');
      items.forEach((item, index) => {
        const text = NAV_TOOLTIPS[index];
        if (!text) return;

        const link = item.querySelector('[class*="NavItem-link"]') as HTMLElement | null;
        if (!link) return;
...
export function createCSSPlugin(options: CSSPluginOptions): PuckPlugin {
  ...
  /**
   * Preview override — renders a "Viewing a previous version" warning banner
   * above the Puck canvas when the user has selected a historical version.
   */
  function VersionBannerOverride({ children }: { children: React.ReactNode }): React.ReactElement {
    const cssContext = useCSSPuck();
    const versions = stableOptions.versions ?? [];
    const selectedVersionId = stableOptions.selectedVersionId;
          ...
          <div className="pds-section-message pds-section-message--warning" role="status" style={{ borderRadius: 0, flexShrink: 0 }}>
            <div className="pds-section-message__content">
              <div className="pds-section-message__icon">
                <svg aria-hidden="true" className="pds-icon pds-icon--lg" fill="none" focusable="false" height="1.2rem" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
                  <path d="M256 32c14.2 0 27.3 7.5 34.5 19.8l216 368c7.3 12.4 7.3 27.7 .2 40.1S486.3 480 472 480H40c-14.3 0-27.6-7.5-34.7-19.8s-7.2-28-.2-40.1l216-368C228.7 39.5 241.8 32 256 32zm0 128c-13.3 0-24 10.7-24 24V296c0 13.3 10.7 24 24 24s24-10.7 24-24V184c0-13.3-10.7-24-24-24zm32 224a32 32 0 1 0 -64 0 32 32 0 1 0 64 0z" fill="currentColor" fillRule="evenodd"/>
                </svg>
              </div>
            ...
            <div className="pds-section-message__actions">
              <button
                ...
                onClick={() => versions[0] && stableOptions.onVersionSelect?.(versions[0])}
              >
                Return to current
              </button>
            </div>
  ...
  function HeaderOverride(): React.ReactElement {
    ...
    const handleCompareWithLive = stableOptions.onCompareWithLive ?? (() => setShowMergeReview(true));
    const handleCreateBranch = stableOptions.onCreateBranch ?? (async (name: string) => { await css.createBranch(name); });

    return (
      <>
        <P1EditorHeader
          branches={stableOptions.branches}
          currentBranch={stableOptions.currentBranch}
          documents={mappedDocs}
          currentDocument={currentDocMapped}
          siteName={stableOptions.siteName ?? css.siteName ?? ''}
          siteMenuItems={stableOptions.siteMenuItems ?? DEFAULT_SITE_MENU_ITEMS}
          currentUser={currentUser}
          onCreateBranch={handleCreateBranch}
          onSwitchBranch={stableOptions.onBranchSwitch}
          onSelectDocument={(doc) => stableOptions.onDocumentSelect?.(doc.path)}
          onCreateDocument={stableOptions.onDocumentCreate}
  ...
  return {
    name: 'css',
    label: '',
    icon: <NavIcon iconName="rotateClock" tooltip="History" />,
    render: () => (
      <>
        ...
        {/* Tooltip bridge — renders portal tooltips for Blocks/Outline/AI/Articles nav tabs */}
        <NavTooltipBridge />
    ...
    overrides: {
      ...
      preview: ({ children }: { children: React.ReactNode }) => (
        <VersionBannerOverride>{children}</VersionBannerOverride>
      ),
    },
  };
}
