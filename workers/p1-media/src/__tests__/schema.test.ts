...
describe('validateMetadata', () => {
  ...
  it('rejects an unknown field name so callers cannot smuggle arbitrary keys into storage (R13)', () => {
    ...
    expect(result).toEqual({ ok: false, error: expect.stringContaining('Unknown metadata field') });
  });
  ...
  it('rejects a value larger than the per-field byte cap (R6)', () => {
    ...
    expect(result).toEqual({ ok: false, error: expect.stringContaining(String(MAX_METADATA_FIELD_BYTES)) });
  });
  ...
  it('rejects more than the max number of fields (R6)', () => {
    ...
    expect(result).toEqual({ ok: false, error: expect.stringContaining('Too many metadata fields') });
  });
  ...
  it('rejects a non-string, non-null value', () => {
    ...
    expect(result).toEqual({ ok: false, error: expect.stringContaining('must be a string') });
  });
});
