...
export async function loadRemoteDatasourceContext(
  ...
  // Run all builtin fetchers and user-defined datasources in parallel
  const [builtinResults, userResults] = await Promise.all([
    Promise.all(
      builtinFetchers.map(async (f) => [f.id, await f.fetch(fetcherParams)] as const)
    ),
    Promise.all(
      allUserDefs.map(async (def) => [def.id, await fetchHttpJsonRemoteDatasource(def, userContextVars, fetchImpl)] as const)
    ),
  ]);
