...
The P1 block library is distributed as a shadcn code registry, not a package. Blocks are authored in
`packages/p1-starter-components`, served by `apps/p1-registry`, and browsable at
[components.p1.pantheon.io](https://components.p1.pantheon.io/) — which is also where the install
command for each block lives.

```bash
pnpm dlx shadcn@latest add @p1/pricing   # in a scaffolded project, not here
```

The install prints the import, component-key and category lines to paste into
`components/puck/blocks/index.ts`; the catalog card carries the same lines.

Neither this repo nor `apps/p1-starter` carries a `components.json`, so `add` cannot run here — the
blocks are installed into a scaffolded project. See
[create-p1-starter-kit](packages/create-p1-starter-kit/README.md).
