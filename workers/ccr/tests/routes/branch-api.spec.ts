...
describe('Phase 7.1a: Branch API Routes', () => {
  ...
  describe('POST /api/sites/{siteId}/branches', () => {
    it('should create a new branch', async () => {
      ...
      vi.mocked(services.createBranch).mockResolvedValueOnce(makeBranch({
        id: 'new-branch-id',
        siteId: 'site-1',
        name: 'feature-branch',
    ...
    it('should create a branch from a non-main branch using parentBranchId', async () => {
      ...
      expect(body.sourceBranchId).toBe('feature-branch-id');
  ...
  describe('PATCH /api/sites/{siteId}/branches/{branchId}', () => {
    it('should update branch details', async () => {
      ...
      vi.mocked(services.updateBranch).mockResolvedValueOnce(makeBranch({
        id: 'branch-1',
        siteId: 'site-1',
        name: 'updated-name',
    ...
    it('should update branch status', async () => {
      ...
      vi.mocked(services.updateBranchStatus).mockResolvedValueOnce(makeBranch({
        id: 'branch-1',
        siteId: 'site-1',
        name: 'feature-branch',
        isMain: false,
  ...
  describe('DELETE /api/sites/{siteId}/branches/{branchId} — soft delete (PCC-3211)', () => {
    ...
    it('should return 400 when archiving the main branch', async () => {
      ...
      const response = await handleBranchRoutes(
        ...
        { siteId: 'site-1', branchId: 'main-id', principal: makePrincipal({ id: 'user-1', type: 'user' }) },
      );
  ...
  describe('POST /api/sites/{siteId}/branches/{branchId}/restore (PCC-3211)', () => {
    it('should restore an archived branch and return 200 with branch JSON', async () => {
      ...
      const restoredBranch = makeBranch({
        id: 'branch-1',
        siteId: 'site-1',
        name: 'feature',
        isMain: false,
      ...
      const response = await handleBranchRoutes(
        ...
        { siteId: 'site-1', branchId: 'branch-1', action: 'restore', principal: makePrincipal({ id: 'user-1', type: 'user' }) },
      );
    ...
    it('should return 404 when restoreBranch returns null', async () => {
      ...
      const response = await handleBranchRoutes(
        ...
        { siteId: 'site-1', branchId: 'nonexistent', action: 'restore', principal: makePrincipal({ id: 'user-1', type: 'user' }) },
      );
  ...
  // git branch's name to a CCR branch), so write:registry's coarse SCOPE_RULES
