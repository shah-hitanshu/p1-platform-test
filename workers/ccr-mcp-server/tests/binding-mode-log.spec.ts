 ...
 * CCR_BACKEND service binding is not wired up. In that fallback mode the
 ...
 *   1. CCR_BACKEND present  → console.log "service-binding"
 *   2. CCR_BACKEND missing  → console.warn "public-fetch" AND "MISSING"
...
describe('Binding-mode cold-start log', () => {
  ...
  it('logs service-binding mode when CCR_BACKEND fetcher is present', async () => {
    vi.resetModules();
    const { logBindingModeOnce } = await import('../src/binding-mode.js');
    logBindingModeOnce({ CCR_BACKEND: {} as Fetcher });

    expect(logSpy).toHaveBeenCalledTimes(1);
    expect(logSpy.mock.calls[0][0]).toContain('CCR_BACKEND');
    expect(logSpy.mock.calls[0][0]).toContain('service-binding');
    expect(warnSpy).not.toHaveBeenCalled();
  });

  it('warns public-fetch + MISSING when CCR_BACKEND is undefined', async () => {
    vi.resetModules();
    const { logBindingModeOnce } = await import('../src/binding-mode.js');
    logBindingModeOnce({ CCR_BACKEND: undefined });

    expect(warnSpy).toHaveBeenCalledTimes(1);
    const msg = String(warnSpy.mock.calls[0][0]);
    expect(msg).toContain('CCR_BACKEND');
