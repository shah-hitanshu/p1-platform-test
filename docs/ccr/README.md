# Collaborative Content Repository

> **Naming note:** CCR (Collaborative Content Repository) was formerly known as CSS (Collaborative State System). Historical documents in this directory (plans, proposals, PROGRESS.md) retain the old name.
...
The Collaborative Content Repository provides:
...
│   ├── wrangler.jsonc                # Cloudflare Worker config (CCR API)
...
│   │       └── services/
│   │           └── site-lookup.ts    # Service-binding call to CCR API
│   │
...
│       ├── cloudflare/               # KV, Queue, Hyperdrive for CCR API worker
...
The Collaborative Content Repository ships a remote MCP server that lets Claude Desktop and other MCP clients read and edit content using your Pantheon identity. Authentication is handled via OAuth 2.0 — no API keys to manage.
...
> **Legacy local stdio server:** `examples/ccr-mcp/` contains the original agent-key-based stdio MCP server. It is retained for local development reference but is superseded by the remote server for all deployed environments.

---

## CCR Auth Server (OAuth 2.0 for Consuming Sites)

The CCR Auth Server (`workers/auth-server/`) is a standalone Cloudflare Worker that acts as an OAuth 2.0 Authorization Server for puck-css frontend clients. It proxies Google OIDC so consuming sites never need their own Google Client ID — they authenticate only against the CCR Auth Server.

### How It Works

1. A puck-css client starts an Authorization Code + PKCE flow using `client_id = <site_id>`
2. The auth server looks up the site's `allowedOrigins[]` from the CCR API via a service binding and validates the `redirect_uri`
3. The user is redirected to Google for authentication; the auth server exchanges the code for a token
4. The auth server issues a CCR-signed opaque token (`userId:grantId:secret`) to the client
5. The client sends that token as a `Bearer` token to the CCR API, which validates it via the `CCR_AUTH_SERVER` service binding
...
The CCR Auth Server is not included in the standard local dev stack — puck-css clients in local development use the mock identity provider in `workers/src/index.ts`. To test the auth flow locally, run the auth server with Wrangler:
...
The main CCR worker must have `CCR_AUTH_SERVER` pointing at the local auth server instance (set `CCR_AUTH_SERVER_URL` in `workers/.dev.vars`).
...
- **Service Bindings**: `CCR_AUTH_SERVER` (auth server worker, for token validation)
...
MCP worker (`css-mcp-server-production`) reaches the API over the `CCR_BACKEND` service binding; set its agent/server secret if one is required. The database password is carried by Hyperdrive, not stored as a worker secret.
