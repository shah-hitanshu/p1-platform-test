...
function setSlotArray(data: Data, slot: string, arr: PuckBlock[]): void {
  ...
  (data.zones as Record<string, PuckBlock[]>)[slot] = arr as never;
}
...
export function computeSemanticOps(canonical: Data, edited: Data): SemanticOp[] {
  ...
  for (const id of canonIdList) {
    if (!editIds.has(id)) {
      ...
      const canonPlaced = canonMap.get(id);
      const editPlaced = editMap.get(id);
      if (canonPlaced && editPlaced && canonPlaced.block.type !== editPlaced.block.type) {
        toRemove.add(id);
        toAdd.add(id);
      }
    }
