...
async function handleCreateBranch(
  ...
  // Get latest checkpoint from source branch
  const checkpoint = await getLatestCheckpoint(sourceBranch.id);
  if (checkpoint === null) {
    return errorResponse('No checkpoint found on source branch', 400);
  }
