...
describe('useResolvePermissions', () => {
  ...
  it('grants VIEWER with read-only perms (canView=true)', async () => {
    ...
    const { result } = renderHook(() => useResolvePermissions({ client, siteId: 's', branchId: 'b' }));
    // VIEWER has canView=true → granted, with canEditDocuments=false
    await waitFor(() => expect(result.current.outcome).toBe('granted'));
  ...
  it('stays pending with empty branchId', async () => {
    const client = makeClient(async () => ({ roleName: 'EDITOR', permissions: EDITOR_PERMS }));
    const { result } = renderHook(() => useResolvePermissions({ client, siteId: 's', branchId: '' }));
  ...
  it('drops a granted outcome when branchId goes falsy', async () => {
    ...
    const { result, rerender } = renderHook(
      ({ branchId }: { branchId: string }) => useResolvePermissions({ client, siteId: 's', branchId }),
