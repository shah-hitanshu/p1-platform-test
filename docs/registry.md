...
Four details make that work in a monorepo:
...
- **Build output goes to the repo root.** Pantheon collects static assets from
  the repo root only, so `distDir` points there instead of copying `.next/static`
  up after the build. A copy is a snapshot: it and the HTML `next start` serves
  drift apart, and any chunk whose hash changed between them 404s with no
  fallback to the Node server. Turbo cannot track outputs above a package, so
  the registry build opts out of the cache rather than replaying a hit that
  restores nothing.
