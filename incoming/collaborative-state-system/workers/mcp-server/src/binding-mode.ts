/**
 * CSS_BACKEND binding-mode cold-start log (PCC-3193 / red-team Finding 6).
 *
 * The MCP server's api-client falls back to global fetch() whenever
 * env.CSS_BACKEND is undefined, which means the shared agent API key
...
export function logBindingModeOnce(env: { CSS_BACKEND?: Fetcher }): void {
  if (bindingModeLogged) return;
  bindingModeLogged = true;

  if (env.CSS_BACKEND !== undefined) {
    console.log('CSS_BACKEND binding: service-binding (fetcher present)');
  } else {
    console.warn(
      'CSS_BACKEND binding: public-fetch (CSS_BACKEND fetcher MISSING ' +
