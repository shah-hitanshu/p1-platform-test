...
export const TEMPLATE_FUNCTIONS: Record<string, SafeTemplateFn> = {
  ...
  padStart: ([value, length, fill]) => {
    ...
    const capped = Math.min(len, MAX_PAD_LENGTH);
    if (fill !== undefined && typeof fill !== "string") return "";
    return fill === undefined
      ? toText(value).padStart(capped)
      : toText(value).padStart(capped, fill as string);
  },
  padEnd: ([value, length, fill]) => {
    ...
    if (fill !== undefined && typeof fill !== "string") return "";
    return fill === undefined
      ? toText(value).padEnd(capped)
      : toText(value).padEnd(capped, fill as string);
  },
