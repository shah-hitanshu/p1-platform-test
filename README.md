...
| Run p1-starter against staging | `pnpm dev:starter:staging` — one-time setup: copy `.env.staging.example` → `.env.staging.local` (both at repo root) with staging site id + `CSS_API_KEY`. A profile must set every key (even blank, e.g. `NEXT_PUBLIC_CSS_AUTH_MODE=`) so values from a plain `.env.local` can't leak in |
...
**Where env files live:** stack profiles (`.env.fullstack.local`, `.env.staging.local`) sit at repo root — they describe which backend the stack talks to, and `dotenv-cli` can load them from anywhere. Three files are pinned in place by tool conventions and can't move: `apps/p1-starter/.env.local` (Next.js only auto-loads it from the app dir), `workers/collaborative-state/.dev.vars` (wrangler; generated by `make dev-vars`), and `workers/p1-agent/.env` (agent secrets, read next to its wrangler config).
