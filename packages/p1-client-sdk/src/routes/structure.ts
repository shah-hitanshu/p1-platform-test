...
const structureCreators: Record<StructureKind, (path: string) => { ok: true; path: string } | { ok: false; error: string }> = {
...
export async function postStructure(request: Request, kind: StructureKind) {
  ...
  const result = structureCreators[kind](path);
  if (result.ok === false) {
...
export async function deleteStructurePage(request: Request) {
  ...
  const result = deletePageAtPath(path);
  if (result.ok === false) {
    return NextResponse.json({ ok: false, error: result.error }, { status: 400 });
  }
