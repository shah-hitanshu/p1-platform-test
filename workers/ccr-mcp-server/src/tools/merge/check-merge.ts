...
const CheckMergeInputSchema = z.object({
  ...
  target_branch_id: z.string().describe('The workstream ID (UUID) to merge into (often the main workstream)'),
});

export const checkMergeTool = defineTool({
  description:
    'Check whether a source workstream can merge cleanly into a target workstream, and report any conflicting documents. Run this before execute_merge so you know whether conflict resolutions are needed. Read-only — it does not change anything.',
