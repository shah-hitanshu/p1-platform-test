...
const ListDocumentsInputSchema = z.object({
  ...
  branch_id: z.string().describe('The workstream ID (UUID from list_branches, NOT the workstream name)'),
});

export const listDocumentsTool = defineTool({
  description:
    'List all documents in a site workstream. Returns document paths (e.g., "/home", "/about") and their IDs. Use the document path when calling get_document or starting edit sessions.',
  ...
  handler: async (ctx, input) => {
    try {
      ...
      if (contentDocs.length === 0) {
        return formatResult('No documents found in this workstream.');
      }
