...
- Production typecheck, lint, and every unit suite are green through turbo, so a red result there is yours. The one standing accommodation is `workers/collaborative-state`'s test-only type errors, held to a committed ceiling by `pnpm check:typecheck-tests`; that count may fall, never rise.
