-- Carried from numbered migration 073, which landed on main after the
-- cut-over. Every statement is idempotent: a database that ran 073 as a
...
ALTER TABLE "app"."document_relations" ADD COLUMN IF NOT EXISTS "synced_version_id" uuid;--> statement-breakpoint

DO $$
BEGIN
  ALTER TABLE "app"."document_relations" ADD CONSTRAINT "document_relations_synced_version_id_fkey" FOREIGN KEY ("synced_version_id") REFERENCES "app"."document_versions"("id") ON DELETE set null ON UPDATE no action;
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;--> statement-breakpoint

-- Removing a version nulls every pin to it, which scans this column.
CREATE INDEX IF NOT EXISTS "idx_document_relations_synced_version_id" ON "app"."document_relations" USING btree ("synced_version_id") WHERE (synced_version_id IS NOT NULL);--> statement-breakpoint
...
UPDATE app.document_relations dr
 ...
 WHERE dr.relation_type = 'localization'
   ...
   AND dr.synced_version_id IS NULL
