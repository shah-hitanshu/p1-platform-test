...
describe('Phase 10.2: Metrics Service', () => {
  ...
  describe('incrementCounter', () => {
    ...
    it('should buffer counter metric with custom value', async () => {
      ...
      incrementCounter('ccr_http_request_total', { method: 'GET' }, 5);
  ...
  describe('recordTiming', () => {
    it('should buffer timing metric with duration', async () => {
      ...
      recordTiming('ccr_http_request_duration_ms', 150.5);

      const buffer = getMetricsBuffer();
      expect(buffer.length).toBe(1);
      expect(buffer[0]).toMatchObject({
        name: 'ccr_http_request_duration_ms',
        type: 'histogram',
        value: 150.5,
      });
    });
    ...
    it('should buffer timing metric with labels', async () => {
      ...
      recordTiming('ccr_http_request_duration_ms', 250, {
        method: 'GET',
        path_pattern: '/health',
        status_class: '2xx',
      });
  ...
  describe('setGauge', () => {
    it('should buffer gauge metric with value', async () => {
      ...
      setGauge('ccr_ws_connections_active', 42);

      const buffer = getMetricsBuffer();
      expect(buffer.length).toBe(1);
      expect(buffer[0]).toMatchObject({
        name: 'ccr_ws_connections_active',
        type: 'gauge',
        value: 42,
      });
    });
  ...
  describe('flushMetrics', () => {
    ...
    it('should include environment and version labels in all metrics', async () => {
      ...
      incrementCounter('ccr_http_request_total', { method: 'GET' });
  ...
  describe('Multiple metrics per request', () => {
    it('should accumulate multiple metrics in buffer', async () => {
      ...
      incrementCounter('ccr_http_request_total', { method: 'GET', path_pattern: '/api/sites/:id', status_class: '2xx' });
      recordTiming('ccr_http_request_duration_ms', 45.2, { method: 'GET', path_pattern: '/api/sites/:id', status_class: '2xx' });
      setGauge('ccr_db_health_status', 1);
      recordTiming('ccr_db_health_latency_ms', 12.5);

      const buffer = getMetricsBuffer();
      expect(buffer.length).toBe(4);

      // Verify each metric type is present
      expect(buffer.find((m) => m.name === 'ccr_http_request_total')).toBeDefined();
      expect(buffer.find((m) => m.name === 'ccr_http_request_duration_ms')).toBeDefined();
      expect(buffer.find((m) => m.name === 'ccr_db_health_status')).toBeDefined();
      expect(buffer.find((m) => m.name === 'ccr_db_health_latency_ms')).toBeDefined();
    });
