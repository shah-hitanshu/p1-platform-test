...
**[Fix]** The chat panel is now Pantheon Agent, and its surfaces render in the editor's black-and-white palette instead of blue, so the panel no longer reads as a separate product sitting inside the editor chrome.

### What Changed

- The assistant is called Pantheon Agent: the panel title reads "Pantheon Agent" and the composer placeholder reads "Ask Pantheon Agent…".
...
Update anything that selects the panel by its old name — `getByText('Pantheon AI')` and the like — to "Pantheon Agent".
