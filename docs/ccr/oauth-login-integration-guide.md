...
This guide covers how frontend applications authenticate with the CCR backend using OAuth providers (Google and Auth0) and the `css-client` library.

---

## Prerequisites

- The CCR backend already validates Google OAuth2 ID tokens and Auth0 JWTs. No backend code changes are needed to authenticate users.
...
```
┌──────────────┐     ┌────────────────────┐     ┌──────────────────┐
│   Frontend   │     │  OAuth Provider    │     │   CCR Backend    │
...
3. The frontend sends API requests to CCR with the token in the `Authorization: Bearer <token>` header.
4. The CCR backend decodes the JWT `iss` claim (without verification) to route to the correct identity provider, which then performs full signature verification using the provider's JWKS endpoint.
...
function handleCredentialResponse(response: google.accounts.id.CredentialResponse) {
  ...
  // Store the token and use it for CCR API calls
...
const response = await fetch('https://your-ccr-backend.example.com/api/sites', {
  headers: {
...
The `css-client` package provides authentication abstractions that integrate with the CCR API client.
...
// Use with the CCR client
const client = new CssClient({
  baseUrl: 'https://your-ccr-backend.example.com',
  auth: authProvider,
});
```
...
// Create the CCR client with token auth
const authProvider = createTokenAuth(tokenStorage);
const client = new CssClient({
  baseUrl: 'https://your-ccr-backend.example.com',
  auth: authProvider,
...
The CCR backend requires the following environment variables to enable OAuth providers:
...
When a user authenticates via Google or Auth0, the CCR backend deterministically maps their OAuth subject ID (`sub` claim) to a UUID using UUIDv5. This means:

- The same user always gets the same CCR user ID regardless of when they first authenticate.
- No explicit user registration step is required.
- Google and Auth0 users with the same email address will have **different** CCR user IDs because the mapping is per-provider.
...
```
CCR User ID = UUIDv5(providerNamespace, providerSubjectId)
```

For example, if Google returns `sub: "114823571234567890"`, the CCR user ID will always be the same deterministic UUID derived from `UUIDv5("6ba7b810-...", "114823571234567890")`.

### Implications for Site Roles

After authentication, the user's site access is determined by looking up their CCR user ID in the `user_site_roles` table. A user will have `NO_ACCESS` until a role is explicitly granted for a specific site (via MAS synchronization, local grants, or the collaborator API).
...
|-------|------|-------------|
| `id` | `string` | The user's CCR UUID (derived via UUIDv5 from the provider subject ID) |
...
In non-production environments, the CCR backend provides a mock identity provider for local development and testing.
...
<body>
  ...
  <script>
    const CCR_API = 'https://your-ccr-backend.example.com';
    ...
    async function fetchMe() {
      const token = localStorage.getItem('css_auth_token');
      const resp = await fetch(`${CCR_API}/api/auth/me`, {
        headers: { 'Authorization': `Bearer ${token}` },
      });
